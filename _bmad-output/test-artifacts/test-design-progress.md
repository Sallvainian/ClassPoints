...
##### TC-3 — ~~`useSeatingChart` lacks realtime; ADR-005 §6 target unreachable~~ — RETIRED 2026-05-13 (`KI-3`)

**Status:** RETIRED 2026-05-13. The cross-device seating-chart sync use case was dropped; seating-chart is no longer a realtime domain, and ADR-005 §6 now lists seating-chart under non-realtime (with `classrooms` / `behaviors` / `layout_presets` / user settings).
**Original concern:** `useSeatingChart` was hand-rolled with zero realtime channels, but ADR-005 §6 scoped seating-chart as one of three official realtime domains. The gap blocked cross-device sync E2E tests.
**Resolution:** Drop the cross-device sync requirement, not the gap. `useSeatingChart` stays hand-rolled with no realtime — matching the new target. Any `test.skip` stubs for the blocked scenarios should be DELETED, not unblocked.
**Owner / Timeline:** n/a — retired. `useSeatingChart` still has a deferred TanStack-reshape target (separate work item, no realtime).
...
##### TS-4 — Realtime scope is **exactly 2 domains** with PR-block enforcement

**Evidence:** ADR-005 §6 + `project-context.md:73-91`. PR adding a 3rd channel without ADR update is a **review block**. Count reduced from 3 → 2 on 2026-05-13 when seating-chart was dropped from the realtime list.
**Why it's strong:** Test catalog has a finite, well-defined live-sync surface. Negative scenario ("realtime should NOT fire on classrooms / behaviors / layout_presets / seating-chart tables") is asserted once, at integration level, and stays valid until ADR-005 §6 changes.
...
| ASR-2  | **Realtime scope is exactly 2 domains.** Classrooms, behaviors, layout_presets, seating-chart tables (seating_charts/\_groups/\_seats, room_elements), and user-settings tables MUST NOT emit realtime events to clients. (Seating-chart was originally a third realtime domain; dropped 2026-05-13.) | ADR-005 §6, PRD FR5                                 | **ACTIONABLE** — negative-realtime integration scenario (P1)                                       |
...
| R-09     | TECH     | New realtime channel added without ADR-005 §6 update (3rd-channel rule)                                | 1   | 3   | **3** | DOCUMENT  | P3       | Code-review gate; out of test catalog (structural review concern). Count reduced from 4th → 3rd 2026-05-13 when seating-chart was dropped.                                                    | Doc-only                                      |
| ~~R-10~~ | DATA     | ~~Cross-device seating-chart drift (drag on laptop, smartboard doesn't update) — blocked by KI-3~~     | n/a | n/a | n/a   | RETIRED   | n/a      | **RETIRED 2026-05-13** — cross-device seating-chart sync use case dropped; no longer a risk. Any `test.skip` stubs for this scenario should be deleted.                                       | n/a                                           |
...
| Retired   | (n/a)     | 1      | R-10 (RETIRED 2026-05-13 — seating-chart cross-device sync use case dropped) |
| **Total** |           | **20** |                                                                              |
...
**Risks retired:** **R-10** (seating-chart cross-device sync) was retired 2026-05-13 when the cross-device sync use case was dropped. The previously-authored `test.skip` stub for SEAT.01-E2E-06 should be DELETED, not unblocked. `useSeatingChart` still has a deferred TanStack-reshape target, but no realtime dependency.
...
| RT.01-INT-07 | Subscribe to each of `seating_charts` / `seating_groups` / `seating_seats` / `room_elements` → no events ever fire (seating-chart dropped from realtime list 2026-05-13) | INT   | P1       | ASR-2       | Negative-realtime-scope test for the seating tables specifically.             |
...
Risks covered: R-08 indirectly. (~~R-10 cross-device sync~~ — RETIRED 2026-05-13.)
ASRs covered: ASR-1.
**Constraint:** `useSeatingChart` is hand-rolled with NO realtime, and that matches the target as of 2026-05-13 — the cross-device drag-sync use case was dropped. Single-device persistence scenarios get full coverage; cross-device sync scenarios are no longer part of the target.
...
| ~~SEAT.01-E2E-06~~ | ~~Two browsers, same user: drag in browser A → browser B updates within 2s~~                                       | E2E   | RETIRED  | n/a         | **RETIRED 2026-05-13** — cross-device seating-chart sync use case dropped. Delete any existing `test.skip` stub for this scenario. |
...
| **P2**     | 21                                                                |
| **P3**     | 4                                                                 |
| Retired    | 1 (R-10 / SEAT.01-E2E-06 — RETIRED 2026-05-13)                    |
...
- ~~**R-10 / SEAT.01-E2E-06**~~ — **RETIRED 2026-05-13**; the cross-device seating-chart sync use case was dropped. Delete the test.skip stub if it exists.
