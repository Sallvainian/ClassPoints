...
| **R-02** | TECH     | `useSeatingChart` drag-state split regression       | **6** | T-14 runtime `supabase.getChannels().length === 2` (seating-chart dropped 2026-05-13); T-15 manual smoke (single-device drag); T-16 grep |
...
| **T-14** | 5     | Runtime `supabase.getChannels().length === 2` with topics `['students', 'point_transactions']` (seating-chart dropped from realtime list 2026-05-13)                                         | Vitest (or manual) | R-02 / FR5 / NFR6      | Architecture Hook #18. Consumes realtime harness. Fallback is manual DevTools WS-frame smoke if harness can't produce reliably          |
...
| **T-15** | 5     | Phase 5 seating smokes: drag a seat; cancel an in-flight drag; save layout preset mid-rearrangement. (Mid-drag realtime event scenario removed 2026-05-13 — seating-chart is no longer a realtime domain.) | Manual       | R-02      | PRD §Phase 5 acceptance                                                                              |
...
| T-15: Phase 5 seating-chart smokes (single-device; mid-drag realtime scenario removed 2026-05-13)   | Sallvain | Phase 5      | PRD §Phase 5                                                                                                            |
...
| **Seating chart editor**     | Hook split (Phase 5)                                              | Drag latency (single-device). FR18 mid-drag realtime obsolete 2026-05-13. | T-14 channel count (=2, NOT 3) + T-15 manual single-device drag smoke   |
