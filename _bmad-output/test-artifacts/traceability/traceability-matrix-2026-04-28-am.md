...
tempCoverageMatrixPath: '/tmp/tea-trace-coverage-matrix-2026-04-28T14-00-33-000Z.json'
...
| --- | ----------------------------------------------------- | --------- | ----- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `src/hooks/__tests__/useAwardPoints.test.ts`          | UNIT      | 4     | ✅ all passing (Wave 1a)                   | AWARD.01-UNIT-01, -02, -03, -04 (explicit, per `automation-summary.md`)                                                                    |
| 2   | `src/hooks/__tests__/useRealtimeSubscription.test.ts` | UNIT      | 8     | ✅ passing                                 | RT.01-INT-06 partial (subscription cleanup, NFR6); rest are infrastructure tests for the hook itself, not catalog scenarios                |
| 3   | `src/test/leaderboardCalculations.test.ts`            | UNIT      | 30    | ✅ passing                                 | P3 background — `(existing-1)` row in catalog                                                                                              |
| 4   | `src/test/useRotatingCategory.test.ts`                | UNIT      | 8     | ✅ passing                                 | P3 background — `(existing-2)` row, KI-5 debt                                                                                              |
| 5   | `src/test/sounds.test.ts`                             | UNIT      | 17    | ✅ passing                                 | SET.01-UNIT-02 (catalog says "18 tests stay green" — actual is 17; minor delta)                                                            |
| 6   | `src/test/TeacherDashboard.test.tsx`                  | COMPONENT | 13    | ❌ **all failing** (pre-existing)          | Unmapped — these were authored before test-design and overlap with would-be P0/P1 dashboard scenarios; superseded by E2E catalog           |
| 7   | `src/utils/__tests__/studentParser.test.ts`           | UNIT      | 33    | ✅ passing                                 | STUD.01-UNIT-01 (one row, but file has 33 tests covering JSON/CSV/auto-detect/displayName generation — broader coverage than catalog asks) |
...
| E2E       | 1     | 2       | 2       | 0       | 0                                                                                                               | 2                                                                                                                         |
| INT       | 1     | 2       | 2       | 0       | 0                                                                                                               | 2                                                                                                                         |
| UNIT      | 7     | 113     | 100     | 13      | 4 explicit + 4 implicit (incl. `(existing-1)`, `(existing-2)`, SET.01-UNIT-02 cluster, STUD.01-UNIT-01 cluster) | rest are out-of-catalog (TeacherDashboard component tests, broader studentParser coverage, useRealtimeSubscription infra) |
| **Total** | **9** | **117** | **104** | **13**  | **~8 catalog-aligned**                                                                                          | **109 not-on-catalog**                                                                                                    |
...
| 8   | MEDIUM   | Decide disposition of 13 failing TeacherDashboard.test.tsx tests (recommend delete since catalog supersedes with E2E)             | repo cleanup               |
| 9   | LOW      | `bmad-testarch-test-review` to assess existing test quality                                                                       | post-Wave-1b               |
...
- Source: `/tmp/tea-trace-coverage-matrix-2026-04-28T14-00-33-000Z.json` (`tempCoverageMatrixPath` from frontmatter)
...
Existing tests are GREEN where they exist (104 of 117 passing). The 13
red tests in TeacherDashboard.test.tsx are a pre-existing ThemeProvider
bug, scoped out per known-failures.md.
...
|            | 2026-04-22 trace (legacy)         | This trace (2026-04-28)                                                                                   |
| ---------- | --------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Scope      | TanStack Phase 0-1 spec migration | Full ClassPoints feature catalog                                                                          |
| Baseline   | commit `3860463`                  | commit `cd0ad84`                                                                                          |
| Total reqs | 12                                | 94                                                                                                        |
| Decision   | FAIL (automation-gap)             | FAIL (automation-gap)                                                                                     |
| Notes      | Localized scope; small spec       | Comprehensive catalog; the natural starting state immediately after `bmad-testarch-test-design` + Wave 1a |

**The current FAIL is expected and healthy.** It represents the trace establishing a measurable baseline; the cycle from here is `atdd → automate Wave 1b/1c → re-trace → CONCERNS → re-trace → PASS`. Without this baseline, automation has no objective progress signal.

### Outputs written

| File                                                              | Purpose                                    |
| ----------------------------------------------------------------- | ------------------------------------------ |
| `_bmad-output/test-artifacts/traceability/traceability-matrix.md` | This document — full narrative report      |
| `/tmp/tea-trace-coverage-matrix-2026-04-28T14-00-33-000Z.json`    | Phase 1 matrix (handoff to Step 5)         |
