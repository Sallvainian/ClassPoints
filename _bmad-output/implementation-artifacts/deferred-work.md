...
- **RESOLVED in `a7887d7`** — Phase 3 chose option (b). `useStudents` is now the single owner of the `students`-table realtime subscription; its `onChange` invalidates both `students.byClassroom(classroomId)` (via merge-patch) and `classrooms.all`. The `useClassrooms` subscription block was deleted; refresh comes from the upstream invalidation.
