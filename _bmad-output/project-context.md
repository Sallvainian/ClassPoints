...
- Secrets managed via **fnox** with the **age** provider — encrypted in `fnox.toml` (committed), decrypted at runtime via `fnox exec --` which wraps Vite in `package.json` scripts. CI uses `FNOX_AGE_KEY` from GitHub Secrets. Do not re-introduce dotenvx, Doppler, or raw `.env.local` tooling.
