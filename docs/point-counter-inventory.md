...
Reference map of every point-counter rendered or computed in the UI, with its data source and code location. Use this before any migration that touches `students.point_total`, the `get_student_time_totals_all_for_user` RPC (the batched replacement for the dropped per-classroom `get_student_time_totals`, deferred #8), or related fields.
...
| Student today delta (`today_total`)          | **Computed** by RPC `get_student_time_totals_all_for_user` | Returned fresh per query from `point_transactions` in today's window (one batched call, all classrooms) | `useStudents.queryFn` calls the RPC inside TanStack Query, filters rows to its classroom, and merges into the cache; also called once in `useClassrooms.queryFn` |
| Student week delta (`this_week_total`)       | **Computed** by the same RPC                               | Same                                                                                                    | Same                                                                                                                                                             |
| Classroom lifetime net/+/-                   | Aggregated client-side from students                       | Summed in `useClassrooms.queryFn:69-91`                                                                 | `classrooms.all` cache → `ClassroomWithCount`                                                                                                                    |
...
- **`get_student_time_totals_all_for_user` RPC signature or output:** audit **C** (student today render), **F**, **G**, **I**, plus leaderboard `getTodayLeaders` / `getThisWeekLeaders`. Also the two callers: `useStudents.queryFn` and `useClassrooms.queryFn` (one batched call each; `useStudents` filters rows to its classroom, `useClassrooms` keys its map by `classroom_id:student_id`).
- **Transaction trigger behavior:** the column writes AND the realtime event shapes change together. The prior DELETE local-delta handler was deleted (#23) — re-verify the invalidate-only realtime path at `useStudents.ts:41-52` if REPLICA IDENTITY is modified.
