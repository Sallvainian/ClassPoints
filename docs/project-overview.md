...
- **TanStack migration**: `useClassrooms`, `useStudents`, `useTransactions`, `useBehaviors` are migrated, and **Phase 4 dissolved the `AppContext` facade** (server data, wrappers, selectors, and undo machinery moved to direct hooks + four thin transitional modules: `useBatchAward`, `useUndoableAction`, `useAppClassrooms`, `pointSelectors` — the fifth, the in-memory batch-kind map, was deleted by deferred #7 in favor of the `point_transactions.batch_kind` column). `useLayoutPresets` and `useSeatingChart` remain legacy hand-rolled hooks scheduled for migration (deferred #11/#12).
