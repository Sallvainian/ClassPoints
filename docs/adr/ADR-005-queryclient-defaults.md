...
**Rationale.** A single refetch reads the authoritative `students` columns (`point_total` / `positive_total` / `negative_total`) AND re-runs the time-totals RPC (`get_student_time_totals_all_for_user` since #8), so all-time, today, and this-week refresh identically. This eliminates the prior preserve-today/week drift, where the `students`-table UPDATE handler copied the all-time columns from the payload but deliberately preserved today/this-week — so the one realtime event that already refreshed all-time cross-device was intentionally skipping the time totals. Aligns with `modernization-plan.md:61` (callbacks "invalidate the relevant query key") and `prd.md:123` ("Surviving realtime callbacks invoke **only** `queryClient.invalidateQueries` or `queryClient.setQueryData`. No manual `onInsert` / `onUpdate` / `onDelete` state-merging logic remains"). §7 tightens that permission for the live-sync domains specifically — they invalidate only; the PRD's broader `setQueryData` allowance still applies elsewhere (e.g. optimistic mutation `onMutate`).

**Cost.** Watching (non-awarding) screens refetch on each remote award (0 → refetch); since #8 that costs 2 batched `get_student_time_totals_all_for_user` calls per event (one per invalidated queryFn — students + classrooms), O(1) in classroom count. Each batched call scans the caller's entire week-window account rather than a single classroom — a wider per-call scan deliberately traded for O(1) round-trips, trivial at classroom-app scale. The awarding device already invalidates `students.byClassroom` via `useAwardPoints.onSettled` (`useTransactions.ts:231`), so it pays ~0 extra — the realtime echo dedupes against that refetch.

For **bulk operations** the multiplier is higher: `useResetClassroomPoints` / `useClearStudentPoints` (delete N transaction rows) and `useBatchAward` (insert N rows, one per student) drive the trigger `FOR EACH ROW`, so a watching device sees up to O(class size) `students`-table realtime events and refetches. TanStack Query coalesces concurrent invalidations of the same key, which dampens this, but does not eliminate it when the events span more than a single refetch duration (each settled refetch can be re-triggered by the next event). The mitigation lever — deferred **#8** (batch the time-totals RPC) — was PULLED 2026-06-11 (migration `20260611145458_batch_time_totals_rpc.sql`): each event now costs 2 batched RPC calls per watching device (one per invalidated queryFn — students + classrooms), O(1) in classroom count, down from N+1.

**Dependency.** That lever was deferred **#8** (batch the RPC) — pulled 2026-06-11, see the bulk-operations paragraph above. The refetch-amplification follow-up **#24** is RESOLVED by this batching; its optional complement (debouncing/coalescing the invalidation bursts) remains available if burst load ever bites.
