...
The TanStack Query migration's server-data move is essentially done. As of HEAD `c9ca66f` on `main` (2026-06-02), four core domains — `useClassrooms`, `useStudents`, `useTransactions`, `useBehaviors` — are fully migrated, and **Phase 4 (commit `d8cde26`) dissolved the `AppContext` server-data facade**: `AppContext.tsx` is now 33 LOC of UI/session state only (the active-classroom selection), with the old wrappers/selectors/undo-machinery relocated to direct mutation hooks plus five thin transitional modules (`useBatchAward`, `useUndoableAction`, `useAppClassrooms`, `pointSelectors`, `batchKindStore`). Two legacy hooks remain (`useLayoutPresets`, `useSeatingChart`); they are migration targets, not templates. `useAwardPoints` is the canonical optimistic-mutation showcase (3 cache patches: transactions + classrooms + students).
...
| State         | Server state in TanStack Query (4 migrated domains, 2 legacy). UI state in component-local React state. `AppContext` holds ONLY the active-classroom selection (Phase 4 dissolved its server-data facade). Batch correlation + undo machinery live in thin modules (`batchKindStore`, `useUndoableAction`).                                                                                                                                                                                                                          |
...
- The dissolved facade's responsibilities were relocated: mutation fan-out → `useBatchAward` (`awardClass`/`awardSubset`); undo window + batch-kind map → `useUndoableAction` + `batchKindStore`; camelCase bridges → `useAppClassrooms`/`useActiveClassroom` + `dbClassroomToApp`/`dbStudentToApp`; point/transaction selectors → `pointSelectors.ts`; the ~20 imperative wrappers → direct TanStack mutation hooks.
- **Silent-partial-failure nuance (audit cluster #2, REAL sev 5 — STILL OPEN, relocated)**: `useBatchAward`'s `awardClass`/`awardSubset` run per-student `Promise.all` where each `mutateAsync(...).catch(...)` `console.error`s the rejection and returns `null` (`useBatchAward.ts:62-65`/`:107-110`); the orchestrator filters nulls (`:69`/`:114`) and returns only successes, so per-item failures are logged but never reach the caller/UI. The header comment preserves this verbatim as a Phase-4 SPEC non-goal. Do NOT infer a clean throw-on-failure contract at the batch call sites.
...
- NOT clone the Phase-4 transitional modules (`useBatchAward`, `useUndoableAction`, `useAppClassrooms`, `batchKindStore`, `pointSelectors`) as new patterns
...
- **Unit (Vitest)**: 17 active `*.test.{ts,tsx}` files under `src/test/`, `src/hooks/__tests__/`, `src/utils/__tests__/`, `src/types/`, and `src/contexts/`. Coverage includes leaderboard math, sound synthesis/settings, rotating-category timer, student parser, realtime subscription wiring, `useStudents` `onReconnect` refresh contract, `useBehaviors`, `useAwardPoints` ADR-005 guards, the Phase-4 modules (`useBatchAward`, `useUndoableAction`, `useAppClassrooms`, `transforms`, `pointSelectors`), `AwardPointsModal`, TeacherDashboard rendering, AppProvider, and the `dashboard-students-mount` regression guard (exactly one `useStudents` subscription per dashboard mount).
