...
- **Atomic batch awards (audit cluster #2, REAL sev 5 — FIXED in `30da564`)**: `useBatchAward`'s `awardClass` (`:227`) / `awardSubset` (`:236`) fire ONE atomic multi-row insert via `useAwardPointsBatch` (`useTransactions.ts:262`, bare `.select()` → all-or-none) and **THROW `BatchAwardError`** (`useBatchAward.ts:27`/`:217`) on any failure — the prior per-student `Promise.all` + silent `.catch(() => null)` filter is gone. A bounded recovery re-query (`classifyAndRecover`, `:68`, `AbortSignal.timeout(2000)`) names the offending student(s) and disambiguates a lost ack (CAP-6: a late-confirmed commit is suppressed as success). Genuine failures record a session-ephemeral `failedBatchStore` notice surfaced as a synthetic FAILED activity-feed entry (CAP-3, `activityFeed.ts:18`). Batch call sites CAN now rely on a clean throw contract.
